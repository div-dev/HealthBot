...
class ChatInterface(Frame):
    ...
    # Options
       ...
       # color_theme.add_command(label='Mkbhd',command=self.MKBHD)
        ...
        self.sp_button = Button(self.send_button_frame, text="Speak", width=5, relief=GROOVE, bg='white',
                                  bd=1, command=self.speak, activebackground="#FFFFFF",
                                  activeforeground="#000000")
        self.sp_button.pack(side=LEFT, ipady=8)
        self.entry_field.bind("<Return>", self.send_message_in)
    ...
    def send_message_insert2(self, message):
        ...
        f =  open("log.txt","w+")
        f.write(str(time.strftime( "Sent At: " + '%B %d, %Y' + ' at ' + '%I:%M %p'))+"\n"+user_input+"\n")
        f.close()
        ob=chat(user_input)
        pr="Bot : " + ob + "\n"
        self.text_box.configure(state=NORMAL)
